...
class Point:
	...
	def __init__(self, x0, x1, z, cart, t=None):
		self.x0 = x0 #x, rho, R
		self.x1 = x1 #y, phi, th
		self.z = z 	 #z, z, phi
		self.t = t 	 #time, for spacetime calcs	
	...
	#rotate self vector wrt the other vector
	#as an axis by th degrees
	def rotateWRT(self, v, th):
		axis = [v.getX(), v.getY(), v.getZ()]
		axis = np.asarray(axis)
		axis = axis/np.sqrt(np.dot(axis, axis))
		a = np.cos(th/2.0)
		b, c, d = -axis*np.sin(th/2.0)
		aa, bb, cc, dd = a*a, b*b, c*c, d*d
		bc, ad, ac, ab, bd, cd = b*c, a*d, a*c, a*b, b*d, c*d
		rotMat = np.matrix([[aa+bb-cc-dd, 2*(bc+ad), 2*(bd-ac)], \
	                     [2*(bc-ad), aa+cc-bb-dd, 2*(cd+ab)],\
	                     [2*(bd+ac), 2*(cd-ab), aa+dd-bb-cc]])

		newv = self.rotate(rotMat)
		return newv

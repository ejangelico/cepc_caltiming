...
TVector3 ImpactPoint( TVector3 MCP_Mom, int MCP_Charge, float Track_Half_Z, float Track_Radius, float B_Field ) //B_Field always along Z+ direction
{
        ...
        if(MCP_Charge == 0)
        {
                ...
                Tau = std::min( float(fabs(1.0/Ratio_Z_Pz*Track_Half_Z/MCP_Mom.Z())), fabs(Track_Radius/R_helix));
...
std::vector<Cluster*> ClusterAbsorbtion( std::vector<Cluster*> MainClusters, std::vector<Cluster*> FragClusters, float DisThreshold, float DepthSlope )	//ProjectiveDis
{
	...
	for(int i0 = 0; i0 < N_frag; i0 ++)
	{
		...
		for(int j0 = 0; j0 < N_Core; j0++)
		{
			...
			TVector3 CoG_a=ClusterCoG(a_core);
			TVector3 CoG_b=ClusterCoG(a_frag);
			TVector3 CoG_dif=CoG_a-CoG_b;
			CurrFragCoreDis = sqrt(pow(CoG_dif.Mag(),2)-pow(CoG_dif.Z(),2));
...
int PhotonTag(Cluster *a_clu)
{
	...
	for(int i1 = 0; i1 < CluSize; i1++)
	{
		...
		float tmpTime = a_hit->getTime() - 1.0/300*HitPos.Mag();
...
std::vector<float> ClusterTime(Cluster *inputCluster)
{
	...
	int NHit = inputCluster->getCalorimeterHits().size();
	float currhitTime = 0;
	...
	{
		CalorimeterHit * a_hit = inputCluster->getCalorimeterHits()[OriTimeOrder[j0]];
		hitP = a_hit->getPosition();
		if(N_PropTimeHit > 10)
		{
			if( j0 < 5)
				StartP += 0.2*hitP;

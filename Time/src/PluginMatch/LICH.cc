	...
	_output(0)
{
	...
	en_bin.push_back(-1);
	registerProcessorParameter("EnergyBins",
			"energy bins to make cut different",
			en_bin,
			en_bin);
	NEnBin=en_bin.size();
	for(int i=0;i<NEnBin;i++){
		mvacut_e.push_back(0.5);
		mvacut_mu.push_back(0.5);
		mvacut_pi.push_back(0.5);
	}
...
void LICH::init() {
	...
	piTree->Branch("NMCP", &NMCP, "NMCP/I");
	...
	muTree->Branch("NMCP", &NMCP, "NMCP/I");
	...
	evtTree->Branch("minAngle", &minAngle, "minAngle/F");
	evtTree->Branch("minEn", &minEn, "minEn/F");
	evtTree->Branch("LCEn", &LCEn, "LCEn/F");
	evtTree->Branch("LCID", &LCID, "LCID/I");
	evtTree->Branch("TotalEn", &TotalEn, "TotalEn/F");
	evtTree->Branch("FPFO", &FPFO, "FPFO/I");
	evtTree->Branch("FMu", &FMu, "FMu/I");
	evtTree->Branch("FE", &FE, "FE/I");
	evtTree->Branch("MCPDG", &MCPDG, "MCPDG/I");
	evtTree->Branch("NMuon", &NMuon, "NMuon/I");
	evtTree->Branch("NElectron", &NElectron, "NElectron/I");
	evtTree->Branch("IniMu",&IniMu,"IniMu/I");
	evtTree->Branch("IniE",&IniE,"IniE/I");
	evtTree->Branch("mva_e", &mva_e, "mva_e/F");
	evtTree->Branch("mva_mu", &mva_mu, "mva_mu/F");
	evtTree->Branch("mva_pi", &mva_pi, "mva_pi/F");
	evtTree->Branch("LCcosTheta",&LCcosTheta,"LCcosTheta/F");
	...
	ArborPFO = new TTree("ArborPFO","ArborPFO");
	ArborPFO->Branch("EventNr", &eventNr, "EventNr/I");
	ArborPFO->Branch("Num", &Num, "Num/I");
	ArborPFO->Branch("RecoEn", &RecoEn, "RecoEn/F");
	ArborPFO->Branch("LinkMCEn", &LinkMCEn, "LinkMCEn/F");
	ArborPFO->Branch("ImpactPara", &ImpactPara, "ImpactPara/F");
	ArborPFO->Branch("trkD0", &trkD0, "trkD0/F");
	ArborPFO->Branch("sigD0", &sigD0, "sigD0/F");
	ArborPFO->Branch("trkZ0", &trkZ0, "trkZ0/F");
	ArborPFO->Branch("sigZ0", &sigZ0, "sigZ0/F");
	ArborPFO->Branch("PFOID", &PFOID, "PFOID/I");
	ArborPFO->Branch("MCID", &MCID, "MCID/I");
	ArborPFO->Branch("MCMotherID", &MCMotherID, "MCMotherID/I");
	ArborPFO->Branch("muMotherID", &muMotherID, "muMotherID/I");
	ArborPFO->Branch("PFOcosTheta", &PFOcosTheta, "PFOcosTheta/F");
	ArborPFO->Branch("closedPar", &closedPar, "closedPar/F");
	ArborPFO->Branch("closedMCP", &closedMCP, "closedMCP/F");
	ArborPFO->Branch("ELike", &ELike, "ELike/F");
	ArborPFO->Branch("MuLike", &MuLike, "MuLike/F");
	ArborPFO->Branch("PiLike", &PiLike, "PiLike/F");
...
void LICH::NewClusterFlag(Cluster* a_tree, Track* a_trk)
{
	...
	if ( (NHit > 4 && TrackEn > 1) || TrackEn <= 1 )
	{
		...
		if(_Training==0){	
			...
			ClusterID=0;
			int Nenbin=en_bin.size();

			for(int i=0;i<Nenbin-1;i++){
				if(TrackEn>en_bin[i]&&TrackEn<en_bin[i+1]){
				cout<<TrackEn<<" "<<en_bin[i]<<" "<<en_bin[i+1]<<" "<<mvacut_e[i]<<endl;
					if(mvaVal_e>mvacut_e[i]&&mvaVal_mu<mvacut_mu[i]&&mvaVal_pi<mvacut_pi[i]){
						ClusterID = -11;
					}
					if(mvaVal_pi>mvacut_pi[i]&&mvaVal_mu<mvacut_mu[i]&&mvaVal_e<mvacut_e[i]){
						ClusterID = 211;
					}
					if(mvaVal_mu>mvacut_mu[i]&&mvaVal_pi<mvacut_pi[i]&&mvaVal_e<mvacut_e[i]){
						ClusterID = -13;
					}
				}
			}
			if(TrackEn>en_bin[Nenbin-1]){
				cout<<TrackEn<<" "<<en_bin[Nenbin-1]<<" "<<en_bin[Nenbin-1]<<" "<<mvacut_e[Nenbin-1]<<endl;

					if(mvaVal_e>mvacut_e[Nenbin-1]&&mvaVal_mu<mvacut_mu[Nenbin-1]&&mvaVal_pi<mvacut_pi[Nenbin-1]){
						ClusterID = -11;
					}
					if(mvaVal_pi>mvacut_pi[Nenbin-1]&&mvaVal_mu<mvacut_mu[Nenbin-1]&&mvaVal_e<mvacut_e[Nenbin-1]){
						ClusterID = 211;
					}
					if(mvaVal_mu>mvacut_mu[Nenbin-1]&&mvaVal_pi<mvacut_pi[Nenbin-1]&&mvaVal_e<mvacut_e[Nenbin-1]){
						ClusterID = -13;
					}
			}
		}	
...
void LICH::SamplePro( LCEvent * evtP )
{
	if (evtP)
	{
		...
		try{
			...
			{
				MCParticle * a_MCP = dynamic_cast<MCParticle*>(MCP->getElementAt(i));
				TVector3 VtxPos = a_MCP->getVertex();
				TVector3 EndPPos = a_MCP->getEndpoint();
				int _PIDMCP=a_MCP->getPDG();

				
				if( VtxPos.Perp() < 200 && fabs(VtxPos.Z()) < 2350 && (EndPPos.Perp() > 1800 || fabs(EndPPos.Z()) > 2350 )  && fabs(_PIDMCP) != 12 &&  fabs(_PIDMCP) != 14 &&  fabs(_PIDMCP) != 16 )
					
				//if( a_MCP->getDaughters().size() == 0 )
				{
					FPFO++;
					if(abs(_PIDMCP)==13)FMu++;
					if(abs(_PIDMCP)==11)FE++;
				}
			}
			for(int i = 0; i < MCP->getNumberOfElements(); i++)
			{
				...
				TVector3 EndPPos = a_MCP->getEndpoint();
				int _PIDMCP=a_MCP->getPDG();
				float muonEn=a_MCP->getEnergy();
				float muonAngle = abs(a_MCP->getMomentum()[2]/a_MCP->getEnergy());
				if(VtxPos.Perp() < 200 && fabs(VtxPos.Z()) < 2350 && (EndPPos.Perp() > 1800 || fabs(EndPPos.Z()) > 2350 ) && abs(_PIDMCP)==13  && muonAngle<minAngle)
						{
							minAngle=muonAngle;
							minEn=muonEn;
						}
			...
			cout<<"FPFO "<<FPFO<<" NPFO "<<NPFO<<endl;
			cout<<"training"<<_Training<<endl;
			if(FPFO>0&&_Training==1&&NPFO>0){
				LCEn=0;
				TotalEn=0;
				int LCIndex=0;
				for(int i=0;i<NPFO;i++){
					...
					TotalEn+=b_pfo->getEnergy();
					if(b_pfo->getCharge()==0)continue;
					if(b_pfo->getEnergy()>LCEn){
						LCEn=b_pfo->getEnergy();
						LCIndex=i;
					}
					
				}
				ReconstructedParticle * a_pfo=dynamic_cast<ReconstructedParticle*>(RecoCol->getElementAt(LCIndex));
				Track * a_trk(0);
				...
				//if(LCEn/TotalEn>0.9||TotalEn<1e-4){
				if(abs(MCPDG)==211){
					piTree->Fill();
				}
			...
			if(_Training==0){
				...
				//LCCollection *TrkMCLink = evtP->getCollection("RecoMCTruthLink");
				...
				for(int i=0;i<NPFO;i++){
					...
					TotalEn+=b_pfo->getEnergy();
					Track * a_trk(0);
					if(b_pfo->getTracks().size()){
						a_trk=b_pfo->getTracks()[0];
						float trkD0=a_trk->getD0();
						float trkZ0=a_trk->getZ0();
						float sigD0=sqrt(a_trk->getCovMatrix()[0]);
						float sigZ0=sqrt(a_trk->getCovMatrix()[9]);

						ImpactPara=sqrt(trkD0*trkD0/(sigD0*sigD0)+trkZ0*trkZ0/(sigZ0*sigZ0));
					}
					Cluster * a_clu(0);
					
					if(b_pfo->getClusters().size())a_clu= b_pfo->getClusters()[0];
					for(int ic=0;ic<b_pfo->getClusters().size();ic++){
						chargeparticle->addCluster(b_pfo->getClusters()[ic]);
					}
						cout<<b_pfo->getCharge()<<endl;
					if(b_pfo->getCharge()!=0){
						ParticleIDImpl * pid_e=new ParticleIDImpl();
						ParticleIDImpl * pid_mu=new ParticleIDImpl();
						ParticleIDImpl * pid_pi=new ParticleIDImpl();
						...
						chargeparticle->setCovMatrix(b_pfo->getCovMatrix());
						...
						//	if(a_link->getFrom() == b_pfo)
						//	{
						//		MCParticle * b_MCP = dynamic_cast<MCParticle*>(a_link->getTo());
						//		LinkMCEn=b_MCP->getEnergy();
						//		for(int tt=0;tt< MCP->getNumberOfElements();tt++){
						//			MCParticle * c_MCP = dynamic_cast<MCParticle*>(MCP->getElementAt(tt));
						//			TVector3 Mom_b=b_MCP->getMomentum();
						//			TVector3 Mom_c=c_MCP->getMomentum();
						//			TVector3 VtxPos = c_MCP->getVertex();
						//			TVector3 EndPPos = c_MCP->getEndpoint();
	
						//			if(VtxPos.Perp() < 200 && fabs(VtxPos.Z()) < 2350 && (EndPPos.Perp() > 1800 || fabs(EndPPos.Z()) > 2350 ) &&  c_MCP->getEnergy()> 0.1 ){
						//				if(Mom_c.Angle(Mom_b)<closedMCP&&Mom_c.Angle(Mom_b)>1e-10){
						//					closedMCP=Mom_c.Angle(Mom_b);
						//				}
						//			}
						//		}
						//		MCParticle * a_mupar=b_MCP;
						//		if(abs(b_MCP->getPDG())==13){
						//			do{
						//				cout<<"mother pid: "<<a_mupar->getPDG()<<" Energy "<<a_mupar->getEnergy()<<endl;
						//				a_mupar=a_mupar->getParents()[0];
						//			}while(a_mupar->getParents().size()>0&&abs(a_mupar->getPDG())!=211);
						//			muMotherID=a_mupar->getPDG();	
						//		}
						//		
						//		MCParticle * a_par=b_MCP;
						//		do{
						//			cout<<"mother pid: "<<a_par->getPDG()<<" Energy "<<a_par->getEnergy()<<endl;
						//			a_par=a_par->getParents()[0];
						//		}while(a_par->getParents().size()>0&&abs(a_par->getPDG())!=25);
						//		MCMotherID=a_par->getPDG();	
						//		if(abs(MCMotherID)==13){
						//			IniMu++;
						//		}
						//		if(abs(MCMotherID)==11){
						//			IniE++;
						//		}
						//		MCID = b_MCP->getPDG(); 
						//		cout<<MCID<<" mother pid"<<MCMotherID<<endl;
						//		break; 
						//	}
						//}
						closedPar=999.;
						for(int qq=0;qq<NPFO;qq++){
							ReconstructedParticle * c_pfo=dynamic_cast<ReconstructedParticle*>(RecoCol->getElementAt(qq));
							TVector3 c_mom=c_pfo->getMomentum();
							TVector3 b_mom=b_pfo->getMomentum();
							if(c_mom.Angle(b_mom)<closedPar&&c_mom.Angle(b_mom)>1e-10){
								closedPar=c_mom.Angle(b_mom);
							}
						}
						...
						MuLike=mvaVal_mu;
						PiLike=mvaVal_pi;
						PFOcosTheta=cosTheta;
						RecoEn=b_pfo->getEnergy();
						
						if(b_pfo->getEnergy()>maxEnergy){
							maxEnergy=b_pfo->getEnergy();
							LCcosTheta=cosTheta;
							mva_e=mvaVal_e;
							mva_mu=mvaVal_mu;
							mva_pi=mvaVal_pi;
						...
						otherTree->Fill();
						ArborPFO->Fill();
						if(abs(ClusterID)==13&&abs(MCMotherID)==13){NMuon++;}
						if(abs(ClusterID)==11&&abs(MCMotherID)==11){NElectron++;}
						pid_e->setPDG(int(11*b_pfo->getCharge()));
						pid_mu->setPDG(int(13*b_pfo->getCharge()));
						pid_pi->setPDG(int(-211*b_pfo->getCharge()));
						pid_e->setLikelihood(ELike);
						pid_mu->setLikelihood(MuLike);
						pid_pi->setLikelihood(PiLike);
						chargeparticle->setType(int(ClusterID*b_pfo->getCharge()));
						chargeparticle->addParticleID(pid_e);
						chargeparticle->addParticleID(pid_mu);
						chargeparticle->addParticleID(pid_pi);
						arborrecoparticle->addElement(chargeparticle);
						cout<<"test pid"<<chargeparticle->getParticleIDs()[0]->getPDG()<<endl;
					}
					else{
						chargeparticle->addParticle(b_pfo);
						chargeparticle->setEnergy(b_pfo->getEnergy());
						...
						chargeparticle->setCharge(b_pfo->getCharge());
						chargeparticle->setCovMatrix(b_pfo->getCovMatrix());
						chargeparticle->setType(b_pfo->getType());
						arborrecoparticle->addElement(chargeparticle);
					}
				}
				Num++;
...
void LICH::processEvent( LCEvent * evtP )
{
	...
	if (evtP)
	{
		...
		inputLevel.clear();
		NEn=inputEn.size();
		NPos=inputsubPos.size();
		for(int i=0;i<NEn;i++){
			if(i==0){inputLevel.push_back(-0.1);}
